/**
 * @file   led.c
 * @author Derek Molloy, edited by Emily Dougherty
 * @date   19 April 2015
...
static unsigned int gpioLED = 50;           ///< Default GPIO for the LED is 49
...
static unsigned int gpioLED2 = 14;           ///< Default GPIO for the LED is 49
...
static int flash2(void *arg){
   printk(KERN_INFO "EBB LED: Thread 2 has started running \n");
   while(!kthread_should_stop()){           // Returns true when kthread_stop() is called
      set_current_state(TASK_RUNNING);
      if (mode==FLASH) ledOn2 = !ledOn2;      // Invert the LED state
      else if (mode==ON) ledOn2 = true;
      else ledOn2 = false;
...
static int __init ebbLED_init(void){
   ...
   ebb_kobj2 = kobject_create_and_add("ebb2", kernel_kobj->parent); // kernel_kobj points to /sys/kernel
   if(!ebb_kobj2){
      printk(KERN_ALERT "EBB LED: failed to create kobject\n");
      return -ENOMEM;
   }
   ...
   result = sysfs_create_group(ebb_kobj2, &attr_group);
   if(result) {
   ...
   gpio_direction_output(gpioLED2, ledOn2);   // Set the gpio to be in output mode and turn on
   ...
   task2 = kthread_run(flash2, NULL, "LED_flash_thread2");  // Start the LED flashing thread
   if(IS_ERR(task)){                                     // Kthread name is LED_flash_thread
      printk(KERN_ALERT "EBB LED: failed to create the task\n");
...
static void __exit ebbLED_exit(void){
   ...
   kthread_stop(task2);                    //stop flashing the second LED
   kobject_put(ebb_kobj);                   // clean up -- remove the kobject sysfs entry
   kobject_put(ebb_kobj2); 
